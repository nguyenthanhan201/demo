...
class Designer {
  constructor(store: any) {
    ...
    this.store = store;
    this.getTextAreaVueModel = computed(() => this.store.getters['designer/getTextAreaVueModel']);

    this.getHighlightedImage = computed(() => this.store.getters['designer/getHighlightedImage']);
    this.getHyberlinkEnable = computed(() => this.store.getters['designer/getHyberlinkEnable']);
    this.getElement = computed(() => this.store.getters['designer/getElement']);

    this.getComponents = computed(() => this.store.getters['designer/getComponents']);
    this.getComponent = computed(() => this.store.getters['designer/getComponent']);
    this.getNextSibling = computed(() => this.store.getters['designer/getNextSibling']);
    this.getParentElement = computed(() => this.store.getters['designer/getParentElement']);
    this.getRestoredElement = computed(() => this.store.getters['designer/getRestoredElement']);
  }
  ...
  handleRestoreElement() {
    ...
    if (this.getRestoredElement.value !== null && this.getParentElement.value !== null) {
  ...
  handleFontWeight(userSelectedFontWeight) {
    this.#modifyElementCSS(userSelectedFontWeight, tailwindFontStyles.fontWeight, 'setFontWeight');
  }
  handleFontFamily(userSelectedFontFamily) {
    this.#modifyElementCSS(userSelectedFontFamily, tailwindFontStyles.fontFamily, 'setFontFamily');
  }
  handleFontStyle(userSelectedFontStyle) {
    this.#modifyElementCSS(userSelectedFontStyle, tailwindFontStyles.fontStyle, 'setFontStyle');
  }
  ...
  handleCustomBackgroundColor(userSelectedColor, enabledCustomColor) {
    ...
    if (userSelectedColor === undefined && enabledCustomColor === undefined) {
      ...
      const bgColor = this.getElement.value.style.backgroundColor;
  ...
  handleCustomTextColor(userSelectedColor, enabledCustomColor) {
    ...
    if (userSelectedColor === undefined && enabledCustomColor === undefined) {
      // Get the style property
      const textColor = this.getElement.value.style.color;
  ...
  handleBackgroundColor(userSelectedColor) {
    this.#modifyElementCSS(userSelectedColor, this.backgroundColors, 'setBackgroundColor');
  }
  ...
  handleBackgroundOpacity(opacity) {
    this.#modifyElementCSS(opacity, tailwindOpacities.backgroundOpacities, 'setBackgroundOpacity');
  }
  ...
  addClickAndHoverEvents = () => {
    document.querySelectorAll('section *').forEach((element) => {
      if (this.elementsWithListeners && this.elementsWithListeners.has(element) === false) {
  ...
  observePlusSyncHTMLElements = async () => {
    ...
    this.store.commit('designer/setElement', document.querySelector('[selected]'));
    this.addClickAndHoverEvents();
  };
  ...
  moveComponent(event, dir) {
    ...
    this.getComponents.value.splice(currentIndex, 1);
    this.getComponents.value.splice(currentIndex + 1 * dir, 0, currentComponent);
  ...
  handleTextAreaContent() {
    ...
    if (typeof element.innerHTML === 'string') {
      const textContentElementClone = element.innerHTML.replaceAll('<br>', '\r\n') || '';

      this.store.commit('designer/setTextAreaVueModel', textContentElementClone);
    }
  ...
  async changeText(event) {
    ...
    const textContentWithBr = textContentElementClone.replaceAll(/\r?\n/g, '<br>');
  ...
  previewCurrentDesign() {
    ...
    // commit
    this.store.commit('designer/setCurrentLayoutPreview', addedHtmlComponents.value);
  ...
  areComponentsStoredInLocalStorage(initData?: string) {
    ...
    if (savedCurrentDesign) {
      try {
        this.store.commit('designer/setComponents', JSON.parse(savedCurrentDesign));
      } catch (e) {
  ...
  updateBasePrimaryImage() {
    if (this.getHighlightedImage.value && this.getHighlightedImage.value.file !== null) {
      this.handleDesignerMethods();
      this.store.commit('designer/setBasePrimaryImage', this.getHighlightedImage.value.file);
    }
  }
  ...
  #addHyperlinkToElement(hyperlinkEnable, urlInput, openHyperlinkInNewTab) {
    ...
    if (hyperlinkEnable === true && typeof urlInput === 'string') {
      ...
      if (hyperlink !== null && urlInput.length !== 0) {
        ...
        this.store.commit('designer/setHyperlinkMessage', 'Succesfully updated element hyperlink');
  ...
  handleHyperlink(hyperlinkEnable, urlInput, openHyperlinkInNewTab) {
    ...
    this.#addHyperlinkToElement(hyperlinkEnable, urlInput, openHyperlinkInNewTab);
  }
